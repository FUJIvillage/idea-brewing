...
6. **タップ** — 「タップ」タブで「ビルド開始(1stバッチ)」を押して生成アプリを作り、「注ぐ(サーバー起動)」でローカルdevサーバーを起動。確認後は「止める」で停止。ビルド待ちの間は、ドット絵のループアニメ「真夜中の醸造所」が流れます([PixelForge](https://github.com/FUJIvillage/pixelforge)で制作)
