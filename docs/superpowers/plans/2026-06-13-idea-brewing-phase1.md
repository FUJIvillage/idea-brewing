...
- レシピ生成の最終`refresh()`をtry/catchで包み、ネットワーク失敗時もbusy解除が必ず走るようにした(タブ永久ロック防止)
- auto解除時の`auto:false`送信をループ終了後に一本化し、ループ内のanswer/nextとの`brew.json`書き込み競合(last-writer-wins)を解消。ループ非実行時は従来どおり即時送信
- レシピ409ガードをディスクの`recipeProgress`参照からモジュールスコープの`Set<string>`によるインメモリロックに変更(クラッシュ時の永久ロック残留を防ぎ、初回進捗書き込み前の隙間も閉鎖)
- 原料パネル(原料追加・仕込み)もリフトした`onBusyChange`に接続し、仕込み中のタブ切替でbusy状態が失われないようにした
