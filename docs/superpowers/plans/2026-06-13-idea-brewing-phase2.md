...
レビュー後修正(2026-06-16):
- `prepareBatchDir` のコピー除外判定を絶対パスの部分一致ではなく、テンプレートルートからの相対パスセグメント判定に変更する。チェックアウトパスに `dist` 等が含まれてもテンプレート全体を誤除外しないため。
- `tap.json` の `verify` は文字列配列であることを `readManifest` で検証し、不正なマニフェストは明示エラーにする。
- レシピコピー時の `ENOENT` 以外のエラーは握りつぶさず再throwする。
...
レビュー後修正(2026-06-16):
- `tap/server` の GET と `tap/log` は `readBrew` で存在確認し、ブリュー不在なら 404 `{ error }` を返して既存API契約に揃える。
- `tap/server` の POST は壊れた JSON / `null` body を 400 `{ error: "不正なアクションです。" }` に正規化する。
- `tap/log` はファイル全体ではなく末尾 64KB だけを読み、そこから末尾200行を返す。
- APIルートテストでは `CURSOR_API_KEY` を退避・削除・復元し、未設定テストが環境変数に影響されないようにする。
...
レビュー後修正(2026-06-17):
- `BrewWorkbench` はローカル busy だけでなく `brew.buildProgress !== null` もタブ無効化条件に含める。ページ再読み込み後や別クライアント起動のビルド中に `TapPanel` をアンマウントしてポーリングを止めないため。
- `brew.buildProgress !== null` の間は描画上の表示タブを `tap` に寄せ、`TapPanel` のポーリングが確実に動くようにする。
- `brew-workbench` のユニットテストに、リモートビルド進捗中はタブが disabled になる確認を追加する。
